...
set.seed(0) #It uses to get the same output in every device

split = sample.split(df, SplitRatio = 0.8)
#The above code splits and assign TRUE and FALSE value
...
#if we didn't specify the nvmax, then it will only 8 variables

summary(lm_best)

summary(lm_best)$adjr2 #it gets the adh=justed R^2 values
...
#starts with 10^10 and ends with 10^-2 and gives 100 values
grid
...
optimum_lambda = cv_fit$lambda.min

tss = sum((y-mean(y))^2) #total sum of squares

y_a = predict(lm_ridge, s=optimum_lambda, newx = x)
...
optimum_lambda = cv_fit_l$lambda.min

tss = sum((y-mean(y))^2) #total sum of squares

y_a = predict(lm_lasso, s=optimum_lambda, newx = x)
