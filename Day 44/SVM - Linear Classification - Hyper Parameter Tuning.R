...
set.seed(0)
tune.out = tune(svm, Start_Tech_Oscar~., data = trainclf, kernel = 'linear', ranges = list(cost = c(0.001,0.01, 0.1,1,10,100)))

bestmod = tune.out$best.model
summary(bestmod)

ypredL = predict(bestmod, testclf)
table(ypredL, testclf$Start_Tech_Oscar)
